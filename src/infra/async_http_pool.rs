...
use anyhow::Error;
use eframe::epaint::ahash::RandomState;
...
impl HttpPool{
    pub fn new()->Self{
        ...
        Self{client,buf:Mutex::new(VecDeque::new())}
    }
            ...
            while let Some(i) = lock.pop_front() {
                ...
                tokio::spawn(async move {
                    let function = async move {
                        ...
                        let resp = client.client.request(req).await?;Ok(resp)
                    };

                    let res:Result<anyhow::Result<Response<Body>>,Elapsed> = tokio::time::timeout(i.timeout,function).await;
...
struct HttpHandle{
    ...
    body: Option<Body>,
    handle: Box<dyn FnOnce(anyhow::Result<(StatusCode,HeaderMap,Vec<u8>)>)+ Send+Sync+'static>
}
...
fn http_handle_default(res:anyhow::Result<(StatusCode,HeaderMap,Vec<u8>)>){
...
impl HttpClient{
    ...
    pub fn get<Uri:AsRef<str>>(&self, uri:Uri, headers: HashMap<String, String>, handle:impl FnOnce(anyhow::Result<(StatusCode,HeaderMap,Vec<u8>)>)+ Send+Sync+'static){
        self.request((Method::GET,headers,uri.as_ref(),handle));
    }
    pub fn post<Uri:AsRef<str>,B:Into<Body>>(&self,uri:Uri,headers:HashMap<String,String>,body:B,handle:impl FnOnce(anyhow::Result<(StatusCode,HeaderMap,Vec<u8>)>)+ Send+Sync+'static){
