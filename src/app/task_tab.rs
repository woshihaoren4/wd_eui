...
impl TaskTab {
    pub fn new(coordinate: Coordinate, tasks: Arc<CopyLock<Vec<TaskEntity>>>) -> Self {
    ...
    fn render_tasks(&mut self, _ctx: &Context, ui: &mut Ui) {
    ...
    fn render_create_task(&mut self, ctx: &Context, ui: &mut Ui) {
        ...
        egui::Grid::new("create_task")
            ...
            .show(ui, |ui| {
                ui.label("任务名称:");
                ui.add(
                    egui::TextEdit::singleline(&mut task.name).hint_text("coordinate task name"),
                );
                ui.end_row();
                ui.label("工作总数:");
                ui.add(egui::DragValue::new(&mut task.slot.count).speed(1));
                ui.end_row();
                ui.label(WidgetText::from("节点最大值:").color(Color32::BLUE))
                    .on_hover_text("每个节点能够分配的最大值");
